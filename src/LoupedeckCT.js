...
class LoupedeckCT extends EventEmitter {
	...
	// 60x270
	drawLeftScreen(rgbBuffer, callback) {

		const id = this.responseCallbackAdd(callback);

		let bigbuf1 = Buffer.from('ff1023004c00000000003c010e', 'hex');
		let bigbuf2 = Buffer.alloc(bigbuf1.length+(270*60*2)).fill(0xff);
		let rgbOffset = 0;
		for (let i = 15; i < bigbuf2.length; i += 2) {
			bigbuf2.writeUInt16BE(this.rgb2col(
				rgbBuffer.readUInt8(rgbOffset + 0),
		...
		bigbuf1.copy(bigbuf2, 0, 0, bigbuf1.length);
		let bigbuf3 = Buffer.from('050f23004c', 'hex');
		bigbuf3.writeUInt8(id, 2);
		this.connection.send(bigbuf2);
		this.connection.send(bigbuf3);
	...
	drawRightScreen(rgbBuffer, callback) {

		const id = this.responseCallbackAdd(callback);

		let bigbuf1 = Buffer.from('ff1024005200000000003c010e', 'hex');
		let bigbuf2 = Buffer.alloc(bigbuf1.length+(270*60*2)).fill(0xff);
		let rgbOffset = 0;
		for (let i = 15; i < bigbuf2.length; i += 2) {
			bigbuf2.writeUInt16BE(this.rgb2col(
		...
		bigbuf1.copy(bigbuf2, 0, 0, bigbuf1.length);
		let bigbuf3 = Buffer.from('050f240052', 'hex');
		bigbuf3.writeUInt8(id, 2);
		this.connection.send(bigbuf2);
		this.connection.send(bigbuf3);
...
exports.LoupedeckCT = LoupedeckCT
